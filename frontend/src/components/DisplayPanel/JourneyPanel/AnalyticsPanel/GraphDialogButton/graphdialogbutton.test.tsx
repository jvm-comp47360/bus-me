...
describe(
    '<GraphPredictionButton/> It should be enabled if prediction has been made',
    () => {
      it('should be enabled if prediction and graph predictions are filled',
          () => {
            expect.assertions(1);
            setup(5);
...
describe('<GraphPredictionButton/> It should toggle the graph dialog', () => {
  it('should show the graph dialog when pressed', async () => {
    ...
    const toggleButton: HTMLElement =
      screen.getByRole('button', {name: /journey times/i});
