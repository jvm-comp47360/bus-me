...
def get_prediction(route: str, num_stops_segment: str, time: str) -> Dict[str, float]:
    ...
    return {"prediction": get_prediction_from_pickle_file(num_stops, int(num_stops_segment), planned_duration, route)}


def get_prediction_from_pickle_file(num_stops: int, num_stops_segment: int, planned_duration: int, route: str) -> float:
    """Generate rounded prediction value from provided variables"""
    model = pickle.load(open(get_current_pickle_file_path(route), 'rb'))
...
def get_current_pickle_file_path(route: str) -> str:
